output "fqdn" {
  ...
  value       = length(dns_a_record_set.normal) > 0 ? "${dns_a_record_set.normal[0].name}.${trimsuffix(dns_a_record_set.normal[0].zone, ".")}" : null
}
...
output "fqdn_wildcard" {
  ...
  value       = length(dns_a_record_set.wildcard) > 0 ? "${dns_a_record_set.wildcard[0].name}.${trimsuffix(dns_a_record_set.wildcard[0].zone, ".")}" : null
}
