...
data "netbox_site" "this" {
  for_each = toset([for cluster in var.netbox_clusters : cluster.site_name])
  name     = each.value
}

data "netbox_tenant" "this" {
  for_each = toset([for cluster in var.netbox_clusters : cluster.tenant_name])
  name     = each.value
}

resource "netbox_cluster" "this" {
  for_each        = { for cluster in var.netbox_clusters : cluster.name => cluster }
