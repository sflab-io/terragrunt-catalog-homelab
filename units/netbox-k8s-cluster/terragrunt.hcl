...
provider "netbox" {
  ...
  api_token          = "${include.provider_netbox.locals.netbox_token}"
}
EOF
}

terraform {
  source = "git::git@github.com:sflab-io/terragrunt-catalog-homelab.git//modules/netbox-k8s-cluster?ref=${values.version}"
}
